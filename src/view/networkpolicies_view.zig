...
pub const NetworkPoliciesView = struct {
    ...
    table: TableState(NetworkPolicyInfo),
    ...
    const NetworkPolicyInfo = struct {
        ...
        pub fn deinit(self: *NetworkPolicyInfo) void {
            self.allocator.free(self.name);
            self.allocator.free(self.namespace);
            self.allocator.free(self.pod_selector);
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !NetworkPoliciesView {
        return NetworkPoliciesView{
            ...
            .table = TableState(NetworkPolicyInfo).init(allocator),
        };
    }
    ...
    pub fn refresh(self: *NetworkPoliciesView) !void {
        ...
        const policies = if (self.table.show_all_namespaces)
            self.k8s_service.listAllNetworkPolicies() catch |err| {
                try self.table.setErrorFmt("Failed to list network policies: {}", .{err});
                return;
            }
        else
        ...
        defer self.table.allocator.free(policies);

        for (policies) |np| {
            try self.table.appendItem(.{
                .name = try self.table.allocator.dupe(u8, np.metadata.name),
                .namespace = try self.table.allocator.dupe(u8, np.metadata.namespace orelse "default"),
                .pod_selector = try self.table.allocator.dupe(u8, "<all>"),
                .age = try age_util.calculateAge(self.table.allocator, np.metadata.creationTimestamp),
    ...
    fn applySorting(self: *NetworkPoliciesView) void {
        if (self.table.sort_column) |col| {
            switch (col) {
                COL_NAME => self.table.sortBy(NetworkPolicyInfo.getName),
                COL_AGE => self.table.sortBy(NetworkPolicyInfo.getAge),
                else => {},
            }
        }
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, _: u16, height: u16) !void {
        ...
        for (self.table.filtered_indices.items[range.start..range.end], 0..) |item_idx, i| {
            ...
            try Theme.writeStringWithTheme(term, x + 54, row_y, item.pod_selector[0..@min(18, item.pod_selector.len)], colors.fg, colors.bg);
            try Theme.writeStringWithTheme(term, x + 73, row_y, item.age, colors.fg, colors.bg);
        }
    }
    ...
    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        ...
        switch (key) {
            .char => |c| switch (c) {
                'r' => {
                    self.refresh() catch |err| Logger.err("Failed to refresh network policies: {}", .{err});
                    return .handled;
                },
                '0' => {
