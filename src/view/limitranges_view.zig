...
pub const LimitRangesView = struct {
    ...
    const LRInfo = struct {
        ...
        pub fn deinit(self: *LRInfo) void {
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !LimitRangesView {
        var tbl = TableState(LRInfo).init(allocator);
    ...
    pub fn refresh(self: *LimitRangesView) !void {
        ...
        const lrs = if (self.table.show_all_namespaces)
            self.k8s_service.listAllLimitRanges() catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            }
        else
            self.k8s_service.listLimitRanges(null) catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            };
        defer self.table.allocator.free(lrs);

        for (lrs) |lr| {
            try self.table.appendItem(.{
                .namespace = try self.table.allocator.dupe(u8, lr.metadata.namespace orelse "default"),
                .name = try self.table.allocator.dupe(u8, lr.metadata.name),
                .age = try age_util.calculateAge(self.table.allocator, lr.metadata.creationTimestamp),
    ...
    pub fn getSelectedResourceInfo(self: *LimitRangesView) ?view_mod.ResourceInfo {
    ...
    pub fn applyFilter(self: *LimitRangesView, filter: []const u8) !void {
        try self.table.applyFilter(filter, lrMatchFn);
        self.applySorting();
    }
    ...
    fn applySorting(self: *LimitRangesView) void {
        if (self.table.sort_column) |col| {
            switch (col) {
                COL_NAME => self.table.sortBy(LRInfo.getName),
                COL_AGE => self.table.sortBy(LRInfo.getAge),
                else => {},
            }
        }
    }
    ...
    pub fn createView(self: *LimitRangesView) View {
        return View.create(LimitRangesView, self, &vtable);
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        if (self.table.filtered_indices.items.len == 0) {
            const msg = if (self.table.show_all_namespaces) "No limit ranges found" else "No limit ranges in namespace";
    ...
    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        ...
        switch (key) {
            .char => |c| switch (c) {
                'r' => {
                    self.refresh() catch |err| Logger.err("Failed to refresh limit ranges: {}", .{err});
                    return .handled;
                },
                '0' => {
