...
pub const PodDisruptionBudgetsView = struct {
    ...
    const PDBInfo = struct {
        ...
        pub fn deinit(self: *PDBInfo) void {
            self.allocator.free(self.namespace);
            self.allocator.free(self.name);
            self.allocator.free(self.min_available);
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !PodDisruptionBudgetsView {
        var tbl = TableState(PDBInfo).init(allocator);
    ...
    pub fn refresh(self: *PodDisruptionBudgetsView) !void {
        ...
        const pdbs = if (self.table.show_all_namespaces)
            self.k8s_service.listAllPodDisruptionBudgets() catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            }
        else
            self.k8s_service.listPodDisruptionBudgets(null) catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            };
        defer self.table.allocator.free(pdbs);

        for (pdbs) |pdb| {
            try self.table.appendItem(.{
                .namespace = try self.table.allocator.dupe(u8, pdb.metadata.namespace orelse "default"),
                .name = try self.table.allocator.dupe(u8, pdb.metadata.name),
                .min_available = try self.table.allocator.dupe(u8, "1"),
                .age = try age_util.calculateAge(self.table.allocator, pdb.metadata.creationTimestamp),
    ...
    pub fn applyFilter(self: *PodDisruptionBudgetsView, filter: []const u8) !void {
        try self.table.applyFilter(filter, pdbMatchFn);
        self.applySorting();
    }
    ...
    fn applySorting(self: *PodDisruptionBudgetsView) void {
        if (self.table.sort_column) |col| {
            switch (col) {
                COL_NAME => self.table.sortBy(PDBInfo.getName),
                COL_AGE => self.table.sortBy(PDBInfo.getAge),
                else => {},
            }
        }
    }
    ...
    pub fn createView(self: *PodDisruptionBudgetsView) View {
        return View.create(PodDisruptionBudgetsView, self, &vtable);
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        if (self.table.filtered_indices.items.len == 0) {
            const msg = if (self.table.show_all_namespaces) "No PDBs found" else "No PDBs in namespace";
        ...
        for (self.table.filtered_indices.items[range.start..range.end], 0..) |idx, i| {
            ...
            const line = try std.fmt.allocPrint(self.table.allocator, "  {s: <20} {s: <20} {s: <14} {s}", .{
                if (item.namespace.len > 20) item.namespace[0..20] else item.namespace,
                if (item.name.len > 20) item.name[0..20] else item.name,
                if (item.min_available.len > 14) item.min_available[0..14] else item.min_available,
                item.age,
            });
    ...
    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        ...
        switch (key) {
            .char => |c| switch (c) {
                'r' => {
                    self.refresh() catch |err| Logger.err("Failed to refresh PDBs: {}", .{err});
                    return .handled;
                },
                '0' => {
