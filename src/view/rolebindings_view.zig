...
pub const RoleBindingsView = struct {
    ...
    fn applySorting(self: *RoleBindingsView) void {
        if (self.sort_column) |col| {
            switch (col) {
                COL_NAME => sort_util.sortFilteredIndices(RBInfo, self.items.items, &self.filtered_indices, RBInfo.getName, self.sort_ascending),
                COL_AGE => sort_util.sortFilteredIndices(RBInfo, self.items.items, &self.filtered_indices, RBInfo.getAge, self.sort_ascending),
                else => {},
            }
        }
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        if (self.filtered_indices.items.len == 0) {
            const msg = if (self.items.items.len == 0)
                (if (self.show_all_namespaces) "No role bindings found" else "No role bindings in namespace")
            else
                "No matching role bindings";
        ...
        while (row < self.visible_rows and fi < self.filtered_indices.items.len) : ({
            ...
            const line = try std.fmt.allocPrint(self.allocator, "  {s: <20} {s: <20} {s: <12} {s}", .{
                ...
                if (item.role.len > 12) item.role[0..12] else item.role,
                item.age,
            });
