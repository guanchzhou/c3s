...
pub const EventsView = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !EventsView {
        return EventsView{ .allocator = allocator, .theme = theme, .k8s_service = k8s_service, .items = .{}, .selected_row = 0, .scroll_offset = 0, .loading = false, .error_message = null, .filter_text = "", .show_all_namespaces = true, };
    }
    ...
    pub fn refresh(self: *EventsView) !void {
        ...
        const events = if (self.show_all_namespaces)
            self.k8s_service.listAllEvents() catch |err| { self.error_message = try std.fmt.allocPrint(self.allocator, "Failed: {}", .{err}); return; }
        else
            self.k8s_service.listEvents(null) catch |err| { self.error_message = try std.fmt.allocPrint(self.allocator, "Failed: {}", .{err}); return; };
    ...
    pub fn createView(self: *EventsView) View { return View.create(EventsView, self, &vtable); }
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        const self: *EventsView = @ptrCast(@alignCast(ptr)); _ = width;
        if (self.loading) { try Theme.writeStringWithTheme(term, x, y, "Loading events...", self.theme.main_fg, self.theme.main_bg); return; }
        ...
        if (self.items.items.len == 0) {
            const msg = if (self.show_all_namespaces) "No events found" else "No events in namespace";
            try Theme.writeStringWithTheme(term, x, y, msg, self.theme.main_fg, self.theme.main_bg); return;
        }
        try Theme.writeStringWithTheme(term, x, y, "  NAMESPACE         NAME              TYPE     REASON        MESSAGE           AGE", self.theme.title, self.theme.main_bg);
        ...
        while (row < visible_rows and idx < self.items.items.len) : ({ row += 1; idx += 1; }) {
            ...
            const line = try std.fmt.allocPrint(self.allocator, "  {s: <16} {s: <16} {s: <8} {s: <12} {s: <16} {s}", .{
                ...
                if (item.message.len > 16) item.message[0..16] else item.message, item.age,
            });
    ...
    fn onShow(ptr: *anyopaque) void { const self: *EventsView = @ptrCast(@alignCast(ptr)); self.refresh() catch |err| { Logger.err("Failed: {}", .{err}); }; }
    fn onHide(ptr: *anyopaque) void { _ = ptr; }
    fn getName(ptr: *anyopaque) []const u8 { _ = ptr; return "events"; }
    ...
    fn deinitView(ptr: *anyopaque) void { const self: *EventsView = @ptrCast(@alignCast(ptr)); self.deinit(); }
