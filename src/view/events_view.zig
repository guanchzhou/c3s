/// EventsView - View for Kubernetes Events
const std = @import("std");
const klient = @import("klient");
const view_mod = @import("../viewmodel/view.zig");
const View = view_mod.View;
const KeyResult = View.KeyResult;
const Key = @import("../core/terminal.zig").Key;
const Terminal = @import("../core/terminal.zig").Terminal;
const Theme = theme_loader;
const theme_loader = @import("../model/theme_loader.zig");
const hints_model = @import("../model/hints.zig");
const k8s_service_mod = @import("../services/k8s_service.zig");
const K8sService = k8s_service_mod.K8sService;
const Logger = @import("../core/logger.zig");
const sort_util = @import("../viewmodel/sort.zig");
const age_util = @import("../viewmodel/age.zig");
const TableState = @import("../ui/table_state.zig").TableState;

pub const EventsView = struct {
    theme: *const theme_loader.ThemeColors,
    k8s_service: *K8sService,
    table: TableState(EventInfo),

    // Sort column indices
    const COL_NAME: u8 = 0;
    const COL_AGE: u8 = 1;

    const EventInfo = struct {
        namespace: []const u8,
        name: []const u8,
        event_type: []const u8,
        reason: []const u8,
        message: []const u8,
        age: []const u8,
        allocator: std.mem.Allocator,

        pub fn deinit(self: *EventInfo) void {
            self.allocator.free(self.namespace);
            self.allocator.free(self.name);
            self.allocator.free(self.event_type);
            self.allocator.free(self.reason);
            self.allocator.free(self.message);
            self.allocator.free(self.age);
        }

        fn getName(self: *const EventInfo) []const u8 { return self.name; }
        fn getAge(self: *const EventInfo) []const u8 { return self.age; }
    };

    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !EventsView {
        var tbl = TableState(EventInfo).init(allocator);
        tbl.show_all_namespaces = true;
        return EventsView{
            .theme = theme,
            .k8s_service = k8s_service,
            .table = tbl,
        };
    }

    pub fn deinit(self: *EventsView) void {
        self.table.deinit();
    }

    pub fn refresh(self: *EventsView) !void {
        self.table.loading = true;
        defer self.table.loading = false;
        self.table.clearItems();

        if (!self.k8s_service.isConnected()) {
            try self.table.setError("Not connected");
            return;
        }

        const events = if (self.table.show_all_namespaces)
            self.k8s_service.listAllEvents() catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            }
        else
            self.k8s_service.listEvents(null) catch |err| {
                try self.table.setErrorFmt("Failed: {}", .{err});
                return;
            };
        defer self.table.allocator.free(events);

        for (events) |ev| {
            try self.table.appendItem(.{
                .namespace = try self.table.allocator.dupe(u8, ev.metadata.namespace orelse "default"),
                .name = try self.table.allocator.dupe(u8, ev.metadata.name),
                .event_type = try self.table.allocator.dupe(u8, "Normal"),
                .reason = try self.table.allocator.dupe(u8, "Created"),
                .message = try self.table.allocator.dupe(u8, "Event message"),
                .age = try age_util.calculateAge(self.table.allocator, ev.metadata.creationTimestamp),
                .allocator = self.table.allocator,
            });
        }

        try self.applyFilter(self.table.filter_text);
    }

    pub fn getSelectedResourceInfo(self: *EventsView) ?view_mod.ResourceInfo {
        const item = self.table.getSelectedItem() orelse return null;
        return view_mod.ResourceInfo{
            .name = item.name,
            .namespace = item.namespace,
        };
    }

    pub fn applyFilter(self: *EventsView, filter: []const u8) !void {
        try self.table.applyFilter(filter, matchFn);
        self.applySorting();
    }

    fn applySorting(self: *EventsView) void {
        if (self.table.sort_column) |col| {
            switch (col) {
                COL_NAME => self.table.sortBy(EventInfo.getName),
                COL_AGE => self.table.sortBy(EventInfo.getAge),
                else => {},
            }
        }
    }

    fn matchFn(item: *const EventInfo, filter: []const u8) bool {
        return std.mem.indexOf(u8, item.name, filter) != null or
            std.mem.indexOf(u8, item.namespace, filter) != null;
    }

    pub fn createView(self: *EventsView) View {
        return View.create(EventsView, self, &vtable);
    }

    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        _ = width;
        self.table.visible_rows = if (height > 1) height - 1 else 0;

        if (try self.table.renderStatus(term, x, y, self.theme)) return;

        if (self.table.filtered_indices.items.len == 0) {
            const msg = if (self.table.items.items.len == 0)
                (if (self.table.show_all_namespaces) "No events found" else "No events in namespace")
            else
                "No matching events";
            try Theme.writeStringWithTheme(term, x, y, msg, self.theme.main_fg, self.theme.main_bg);
            return;
        }

        const name_ind = sort_util.sortIndicator(self.table.sort_column, self.table.sort_ascending, COL_NAME);
        const age_ind = sort_util.sortIndicator(self.table.sort_column, self.table.sort_ascending, COL_AGE);
        var name_hdr_buf: [32]u8 = undefined;
        var age_hdr_buf: [16]u8 = undefined;
        const name_hdr = std.fmt.bufPrint(&name_hdr_buf, "NAME{s}", .{name_ind}) catch "NAME";
        const age_hdr = std.fmt.bufPrint(&age_hdr_buf, "AGE{s}", .{age_ind}) catch "AGE";
        var header_buf: [128]u8 = undefined;
        const ev_header = std.fmt.bufPrint(&header_buf, "  NAMESPACE         {s: <18}TYPE     REASON        MESSAGE           {s}", .{ name_hdr, age_hdr }) catch "  NAMESPACE         NAME              TYPE     REASON        MESSAGE           AGE";
        try Theme.writeStringWithTheme(term, x, y, ev_header, self.theme.title, self.theme.main_bg);

        const range = self.table.getVisibleRange();
        for (self.table.filtered_indices.items[range.start..range.end], 0..) |idx, i| {
            const item = self.table.items.items[idx];
            const colors = self.table.rowColors(i, self.theme);
            const row_y = y + 1 + @as(u16, @intCast(i));

            const line = try std.fmt.allocPrint(self.table.allocator, "  {s: <16} {s: <16} {s: <8} {s: <12} {s: <16} {s}", .{
                if (item.namespace.len > 16) item.namespace[0..16] else item.namespace,
                if (item.name.len > 16) item.name[0..16] else item.name,
                if (item.event_type.len > 8) item.event_type[0..8] else item.event_type,
                if (item.reason.len > 12) item.reason[0..12] else item.reason,
                if (item.message.len > 16) item.message[0..16] else item.message,
                item.age,
            });
            defer self.table.allocator.free(line);
            try Theme.writeStringWithTheme(term, x, row_y, line, colors.fg, colors.bg);
        }
    }

    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        const self: *EventsView = @ptrCast(@alignCast(ptr));

        // Common navigation keys
        if (self.table.handleNavigationKey(key)) |result| return result;

        // View-specific keys
        switch (key) {
            .char => |c| switch (c) {
                'r' => {
                    self.refresh() catch |err| Logger.err("Failed to refresh events: {}", .{err});
                    return .handled;
                },
                '0' => {
                    self.table.show_all_namespaces = !self.table.show_all_namespaces;
                    self.table.gotoTop();
                    self.refresh() catch |err| Logger.err("Failed to refresh events: {}", .{err});
                    return .handled;
                },
                'N' => { self.table.toggleSort(COL_NAME); self.applySorting(); return .handled; },
                'A' => { self.table.toggleSort(COL_AGE); self.applySorting(); return .handled; },
                else => return .not_handled,
            },
            else => return .not_handled,
        }
    }

    fn onShow(ptr: *anyopaque) void {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        self.refresh() catch |err| {
            Logger.err("Failed: {}", .{err});
            if (self.table.error_message == null) {
                self.table.setError("Unexpected error during refresh") catch {
                    Logger.err("Failed to allocate error message", .{});
                };
            }
        };
    }

    fn onHide(_: *anyopaque) void {}

    fn getName(_: *anyopaque) []const u8 {
        return "events";
    }

    fn getHints(_: *anyopaque) hints_model.HintConfig {
        return hints_model.resourceHints();
    }

    fn deinitView(ptr: *anyopaque) void {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        self.deinit();
    }

    fn vtableApplyFilter(ptr: *anyopaque, filter: []const u8) anyerror!void {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        try self.applyFilter(filter);
    }

    fn vtableClearFilter(ptr: *anyopaque) anyerror!bool {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        if (self.table.filter_text.len > 0) {
            try self.applyFilter("");
            return true;
        }
        return false;
    }

    fn vtableRefresh(ptr: *anyopaque) anyerror!void {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        try self.refresh();
    }

    fn vtableGetSelectedResource(ptr: *anyopaque) ?view_mod.ResourceInfo {
        const self: *EventsView = @ptrCast(@alignCast(ptr));
        return self.getSelectedResourceInfo();
    }

    const vtable = View.VTable{
        .render = render,
        .handleKey = handleKey,
        .onShow = onShow,
        .onHide = onHide,
        .getName = getName,
        .getHints = getHints,
        .deinit = deinitView,
        .applyFilter = vtableApplyFilter,
        .clearFilter = vtableClearFilter,
        .refresh = vtableRefresh,
        .getSelectedResource = vtableGetSelectedResource,
    };
};
