...
pub const ServiceAccountsView = struct {
    ...
    const SAInfo = struct {
        ...
        pub fn deinit(self: *SAInfo) void {
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !ServiceAccountsView {
        return ServiceAccountsView{
            ...
            .table = TableState(SAInfo).init(allocator),
        };
    }
    ...
    pub fn refresh(self: *ServiceAccountsView) !void {
        ...
        const sas = if (self.table.show_all_namespaces)
            self.k8s_service.listAllServiceAccounts() catch |err| {
                try self.table.setErrorFmt("Failed to list service accounts: {}", .{err});
                return;
            }
        else
        ...
        defer self.table.allocator.free(sas);

        for (sas) |sa| {
            try self.table.appendItem(.{
                .name = try self.table.allocator.dupe(u8, sa.metadata.name),
                .namespace = try self.table.allocator.dupe(u8, sa.metadata.namespace orelse "default"),
                .secrets = 1,
                .age = try age_util.calculateAge(self.table.allocator, sa.metadata.creationTimestamp),
    ...
    fn applySorting(self: *ServiceAccountsView) void {
        if (self.table.sort_column) |col| {
            switch (col) {
                COL_NAME => self.table.sortBy(SAInfo.getName),
                COL_AGE => self.table.sortBy(SAInfo.getAge),
                else => {},
            }
        }
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, _: u16, height: u16) !void {
        ...
        for (self.table.filtered_indices.items[range.start..range.end], 0..) |item_idx, i| {
            ...
            var secrets_buf: [16]u8 = undefined;
            const secrets_str = std.fmt.bufPrint(&secrets_buf, "{d}", .{item.secrets}) catch "?";
            ...
            try Theme.writeStringWithTheme(term, x + 54, row_y, secrets_str, colors.fg, colors.bg);
            try Theme.writeStringWithTheme(term, x + 64, row_y, item.age, colors.fg, colors.bg);
        }
    }
    ...
    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        ...
        switch (key) {
            .char => |c| switch (c) {
                'r' => {
                    self.refresh() catch |err| Logger.err("Failed to refresh service accounts: {}", .{err});
                    return .handled;
                },
                '0' => {
