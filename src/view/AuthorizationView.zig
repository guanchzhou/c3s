...
test "authorization_view: init and cleanup" {
    ...
    try std.testing.expectEqual(@as(usize, 0), view.access_tab.table.items.items.len);
    try std.testing.expectEqual(@as(usize, 0), view.policy_tab.table.items.items.len);
    try std.testing.expectEqual(@as(usize, 0), view.condition_tab.table.items.items.len);
    try std.testing.expectEqual(@as(u32, 0), view.access_tab.table.selected_row);
    try std.testing.expectEqual(@as(u32, 0), view.policy_tab.table.selected_row);
    try std.testing.expectEqual(@as(u32, 0), view.condition_tab.table.selected_row);
    try std.testing.expectEqual(false, view.loading);
    try std.testing.expect(view.error_message == null);
    try std.testing.expect(view.condition_tab.condition_resource == null);
    try std.testing.expectEqual(AuthorizationView.Tab.access_review, view.active_tab);
}
...
test "authorization_view: create view interface" {
    ...
    try std.testing.expect(std.mem.eql(u8, name, "authorization"));
}
...
test "authorization_view: tab switching via keys" {
    ...
    // Switch to tab 2
    _ = try v.handleKey(.{ .char = '2' });
    try std.testing.expectEqual(AuthorizationView.Tab.policy_browser, view.active_tab);

    // Switch to tab 3
    _ = try v.handleKey(.{ .char = '3' });
    try std.testing.expectEqual(AuthorizationView.Tab.condition_inspector, view.active_tab);
...
test "authorization_view: filter with data" {
    ...
    // No filter shows all
    try view.applyFilter("");
    try std.testing.expectEqual(@as(usize, 3), view.access_tab.table.filtered_indices.items.len);
    ...
    try std.testing.expectEqual(@as(usize, 0), view.access_tab.table.filtered_indices.items.len);
...
test "authorization_view: navigation with rows" {
    ...
    try std.testing.expectEqual(@as(usize, 5), view.access_tab.table.filtered_indices.items.len);
    try std.testing.expectEqual(@as(u32, 0), view.access_tab.table.selected_row);

    // Navigate down
    const v = view.createView();
    _ = try v.handleKey(.{ .char = 'j' });
    try std.testing.expectEqual(@as(u32, 1), view.access_tab.table.selected_row);

    _ = try v.handleKey(.{ .char = 'j' });
    _ = try v.handleKey(.{ .char = 'j' });
    try std.testing.expectEqual(@as(u32, 3), view.access_tab.table.selected_row);

    // Navigate up
    _ = try v.handleKey(.{ .char = 'k' });
    try std.testing.expectEqual(@as(u32, 2), view.access_tab.table.selected_row);
    ...
    // Jump to bottom
    _ = try v.handleKey(.{ .char = 'G' });
    try std.testing.expectEqual(@as(u32, 4), view.access_tab.table.selected_row);
}
...
test "authorization_view: getSelectedResourceInfo returns null" {
    ...
    try std.testing.expect(view.getSelectedResourceInfo() == null);
}
...
test "authorization_view: hints are valid" {
    ...
    for (hints.hints) |hint| {
        const rt = @intFromEnum(hint.render_fn);
        try std.testing.expect(rt == 0 or rt == 1);
    }
}

// Test AccessStatus symbols
test "authorization_view: AccessStatus symbols" {
    try std.testing.expect(std.unicode.utf8ValidateSlice(AuthorizationView.AccessStatus.allowed.symbol()));
    try std.testing.expect(std.unicode.utf8ValidateSlice(AuthorizationView.AccessStatus.denied.symbol()));
    try std.testing.expect(std.unicode.utf8ValidateSlice(AuthorizationView.AccessStatus.conditional.symbol()));

    // Verify specific symbols
    try std.testing.expect(std.mem.eql(u8, "\xe2\x9c\x93", AuthorizationView.AccessStatus.allowed.symbol()));
    try std.testing.expect(std.mem.eql(u8, "\xe2\x9c\x97", AuthorizationView.AccessStatus.denied.symbol()));
    try std.testing.expect(std.mem.eql(u8, "~", AuthorizationView.AccessStatus.conditional.symbol()));
}

// Test PolicyType labels
test "authorization_view: PolicyType labels" {
    try std.testing.expect(std.mem.eql(u8, "RBAC", AuthorizationView.PolicyRow.PolicyType.rbac.label()));
    try std.testing.expect(std.mem.eql(u8, "Cedar", AuthorizationView.PolicyRow.PolicyType.cedar.label()));
}
...
test "authorization_view: clear operations" {
    ...
    try std.testing.expectEqual(@as(usize, 1), view.access_tab.table.items.items.len);
    view.clearAccessRows();
    try std.testing.expectEqual(@as(usize, 0), view.access_tab.table.items.items.len);
    ...
    try std.testing.expectEqual(@as(usize, 1), view.policy_tab.table.items.items.len);
    view.clearPolicyRows();
    try std.testing.expectEqual(@as(usize, 0), view.policy_tab.table.items.items.len);
    ...
    try std.testing.expectEqual(@as(usize, 1), view.condition_tab.table.items.items.len);
    view.clearConditionRows();
    try std.testing.expectEqual(@as(usize, 0), view.condition_tab.table.items.items.len);
}
...
test "authorization_view: describe/yaml tab restriction" {
    ...
    try std.testing.expectEqual(View.KeyResult.not_handled, try v.handleKey(.{ .char = 'd' }));
    try std.testing.expectEqual(View.KeyResult.not_handled, try v.handleKey(.{ .char = 'y' }));

    // Tab 2: d/y request describe/yaml
    _ = try v.handleKey(.{ .char = '2' });
    try std.testing.expectEqual(View.KeyResult.request_describe, try v.handleKey(.{ .char = 'd' }));
    try std.testing.expectEqual(View.KeyResult.request_yaml, try v.handleKey(.{ .char = 'y' }));
