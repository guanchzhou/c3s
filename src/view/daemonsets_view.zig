...
pub const DaemonSetsView = struct {
    ...
    pub fn refresh(self: *DaemonSetsView) !void {
        ...
        for (daemonsets) |ds| {
            ...
            const age = try self.allocator.dupe(u8, "1d"); // TODO: Calculate from creationTimestamp
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        // Render header
        const header = "  NAMESPACE             NAME                    DESIRED CURRENT READY UP-TO-DATE AGE";
    ...
    fn handleKey(ptr: *anyopaque, key: Key) !KeyResult {
        ...
        switch (key) {
            .char => |c| switch (c) {
                'j', 'k', 'g', 'G', 'r', '0' => {
                    // Same navigation logic as StatefulSetsView
                    if (c == 'j' and self.items.items.len > 0 and self.selected_row < self.items.items.len - 1) {
                        self.selected_row += 1;
                        ...
                        return .handled;
                    } else if (c == 'k' and self.selected_row > 0) {
                        self.selected_row -= 1;
                        ...
                        return .handled;
                    } else if (c == 'G' and self.items.items.len > 0) {
                        self.selected_row = self.items.items.len - 1;
                        const visible_rows: usize = 20;
            ...
            .down, .up, .page_down, .page_up, .home, .end => {
                // Handle arrow and navigation keys
                if (key == .down and self.items.items.len > 0 and self.selected_row < self.items.items.len - 1) {
                    self.selected_row += 1;
                    const visible_rows: usize = 20;
                    if (self.selected_row >= self.scroll_offset + visible_rows) {
                        self.scroll_offset = self.selected_row - visible_rows + 1;
                    ...
                    return .handled;
                } else if (key == .up and self.selected_row > 0) {
                    self.selected_row -= 1;
                    if (self.selected_row < self.scroll_offset) {
                        self.scroll_offset = self.selected_row;
                    }
