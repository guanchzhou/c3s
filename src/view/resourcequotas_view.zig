...
pub const ResourceQuotasView = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, theme: *const theme_loader.ThemeColors, k8s_service: *K8sService) !ResourceQuotasView {
        return ResourceQuotasView{ .allocator = allocator, .theme = theme, .k8s_service = k8s_service, .items = .{}, .selected_row = 0, .scroll_offset = 0, .loading = false, .error_message = null, .filter_text = "", .show_all_namespaces = true, };
    }
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        try Theme.writeStringWithTheme(term, x, y, "  NAMESPACE             NAME                          AGE", self.theme.title, self.theme.main_bg);
