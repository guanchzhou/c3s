...
pub const ClusterRolesView = struct {
    ...
    fn render(ptr: *anyopaque, term: *Terminal, x: u16, y: u16, width: u16, height: u16) !void {
        ...
        if (self.items.items.len == 0) {
            try Theme.writeStringWithTheme(term, x, y, "No cluster roles found", self.theme.main_fg, self.theme.main_bg);
            return;
        }
        try Theme.writeStringWithTheme(term, x, y, "  NAME                                      AGE", self.theme.title, self.theme.main_bg);
