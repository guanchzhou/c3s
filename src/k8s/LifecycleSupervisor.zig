...
pub const LifecycleSupervisor = struct {
    ...
    fn shutdownToCompletion(self: *LifecycleSupervisor) void {
        ...
        self.discardPendingNodes();

        while (self.live_children != 0 or self.retirementCount() != 0) {
            self.shared_event.reset();
            ...
            self.requestAllCancellations();
            self.discardPendingNodes();
            if (self.live_children == 0 and self.retirementCount() == 0) break;
            if (self.shared_event.isSet()) continue;
    ...
    fn installAndLaunch(
        ...
        self.metrics.max_live = @max(self.metrics.max_live, self.live_children);
    ...
    fn taskMain(_: *LifecycleSupervisor, node: *ChildNode) ChildResult {
    ...
    fn retryPending(self: *LifecycleSupervisor) void {
        for (self.children) |node_optional| {
            ...
            const critical = envelope.target == .lifecycle and self.runtime_failed;
            const pushed = if (critical)
                self.change_queue.tryPushCritical(envelope)
            else
                self.change_queue.tryPushControl(envelope);
    ...
    fn queueDetachedCompletion(self: *LifecycleSupervisor, completion: LifecycleCompletion) void {
        var envelope = completionEnvelope(self.allocator, completion) catch {
            var owned = completion;
            owned.deinit(self.allocator);
            return;
        };
        const result = if (completion == .lifecycle_failed)
            self.change_queue.tryPushCritical(envelope)
        else
            self.change_queue.tryPushControl(envelope);
        result catch envelope.deinit(self.allocator);
    }
    ...
    fn cleanupCanceledBeforeEntry(self: *LifecycleSupervisor, node: *ChildNode) void {
        ...
        node.work = .{ .task = .{ .completion = .{ .context_canceled = 0 } } };
    }
    ...
    fn discardPendingNodes(self: *LifecycleSupervisor) void {
        var slot: usize = 0;
        while (slot < self.children.len) : (slot += 1) {
            const node = self.children[slot] orelse continue;
            if (node.future != null or node.pending == null) continue;
            node.pending.?.deinit(self.allocator);
            node.pending = null;
            self.freeNode(node);
        }
    }
    ...
    fn failRuntime(self: *LifecycleSupervisor, err: anyerror) void {
        ...
        self.shutting_down = true;
        self.inbox.closeNormal();
        self.queueDetachedCompletion(.{ .lifecycle_failed = errorDetail(err) });
    }
...
fn completionEnvelope(
    ...
    const after_revision: ?keys.Revision = switch (completion) {
        .subscription_stopped => std.math.maxInt(keys.Revision),
        else => null,
    };
...
fn completionCommit(completion: *LifecycleCompletion, router: *keys.UiRouter, _: *keys.ApplyPlan) void {
    ...
    if (!data_plane.handleCompletion(completion.*)) return;
...
test "real supervisor start acquires the task selected metrics lease" {
    ...
    while (supervisor.live_children != 0) {
        ...
        if (supervisor.live_children == 0) break;
...
test "one hundred thousand short tasks reuse bounded child storage" {
    ...
    try testing.expectEqual(@as(usize, 0), supervisor.live_children);
    const retired = try slot.invalidate(null);
    retired.?.deinit();
}
