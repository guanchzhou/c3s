...
pub const title = "\x1b[1;38;2;207;201;194m"; // bold light beige/gray for better contrast
pub const app_name = "\x1b[1;97m"; // Bold white for "c3s"
pub const hi_fg = "\x1b[38;2;125;207;255m"; // #7dcfff -> bright cyan
pub const title_highlight = "\x1b[38;2;224;175;104m"; // #e0af68 -> yellow/orange for (all), (default)
...
pub fn writeShortcut(terminal: *Terminal, x: u16, y: u16, key: []const u8, command: []const u8, bg_color: []const u8) !void {
    var buffer: [512]u8 = undefined;
    // Format: <red_key> bold_command
    const formatted = try std.fmt.bufPrint(&buffer, "{s}{s}<{s}>{s} {s}{s}{s}", .{ key_highlight, bg_color, key, reset, bold, command, reset });
    try terminal.setCursor(x, y);
    try terminal.writeAll(formatted);
}

// btop-style shortcut for commands with highlighted letter in middle
pub fn writeShortcutWithHighlight(terminal: *Terminal, x: u16, y: u16, before: []const u8, highlight_char: []const u8, after: []const u8) !void {
